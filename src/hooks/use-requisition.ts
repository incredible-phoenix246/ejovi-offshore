  ...
  devtools(
    (set) => ({
        ...
        set(
          (state) => ({
            currentRequisition: state.currentRequisition
              ? { ...state.currentRequisition, ...updates }
              : null,
          }),
          false,
        ...
        set(
          (state) => ({
            currentRequisition: state.currentRequisition
              ? {
                  ...state.currentRequisition,
                  items: state.currentRequisition.items.filter(
                    (_, i) => i !== index
                  ),
                }
              : null,
        ...
        set(
          (state) => ({
            currentRequisition: state.currentRequisition
              ? {
                  ...state.currentRequisition,
                  items: state.currentRequisition.items.map(
                    (existingItem, i) => (i === index ? item : existingItem)
                  ),
                }
              : null,
          }),
      ...
      setRequisitions: (requisitions) =>
        set({ requisitions }, false, 'setRequisitions'),

      setLoading: (loading) => set({ isLoading: loading }, false, 'setLoading'),

      clearCurrentRequisition: () =>
        set({ currentRequisition: null }, false, 'clearCurrentRequisition'),
    }),
    {
